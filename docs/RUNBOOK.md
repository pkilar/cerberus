...
| `CA_PUBLIC_KEY_PATH`  | —                             | Path to the CA public key (baked into the EIF alongside the ciphertext). **Required in attested/production mode** — the enclave refuses to load if it is unset, or on a public-key mismatch. In dev mode (no `/dev/nsm`) it is optional; omitting it then logs `loadkey.ca_pubkey.unpinned` at WARN. |
...
| `loadkey.ca_pubkey.unpinned` WARN in enclave logs | `CA_PUBLIC_KEY_PATH` not set (dev mode only)               | Dev-only — in attested/production mode an unset `CA_PUBLIC_KEY_PATH` is a hard startup refusal, not a warning. Set it to the EIF-baked CA public key.|
