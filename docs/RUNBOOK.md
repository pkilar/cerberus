...
- **Explicitly** — a `/sign` request with `self_principal: true` (mutually exclusive with `principals`/`all_principals`); the server derives the uid from the authenticated identity and issues a cert for *exactly* that uid. This is what `cssh --self --sign-only` sends.
- **Implicitly, alone** — a normal request whose `principals` are **exactly** the caller's own uid (e.g. `jsmith` requesting `["jsmith"]`) that no group covers is accepted via the self path, using `self_principal.certificate_rules`. The "requested principal equals the authenticated uid" check *is* the verification. This is what makes a plain `cssh jsmith@host` (or `cssh host` when the login equals your uid) connect succeed without group membership.
- **Implicitly, combined with a group grant** — a request naming the caller's own uid *alongside* other principals (e.g. `jsmith` requesting `["jsmith", "root"]`) is accepted as long as some single group covers every principal **other than** the caller's own uid; `self_principal` supplies the caller's own uid for free, riding along on that group's `CertificateRules`. This is what lets a user whose group grants `root` (but not their own uid) request `["<their uid>", "root"]` in one `/sign` call instead of two. A request for any principal that is neither the caller's own uid nor covered by a single group is still refused.
...
- **Denylist** (`self_principal.deny`): short uids that may never be self-issued **by their own principal**, in any of the three ways above. There is **no hardcoded floor on `root`** — self_principal is the intended replacement for static root SSH keys, so a caller whose own uid is `root` (e.g. a Kerberos principal `root@REALM` used for host-to-host automation) is self-issued a `root` cert like any other uid, as long as their realm is allowlisted. Add `root` to `deny` yourself if you want to keep blocking it; add your other shared/role accounts (`deploy`, `postgres`, …) either way. Note this denylist is keyed on the *requesting caller's own uid*, not on principal names in a request — it never blocks `jsmith` from requesting `root` through a group that actually grants it, even if `root` sits on `self_principal.deny`.
- **This path is independent of group membership.** The self-eligibility check (`selfEligibleUID`) never consults Casbin or group config, so no group the caller belongs to — or doesn't belong to — can block or override whether the caller's own uid is self-issuable; the realm allowlist and denylist above are the only gates. Group config only decides what *other* principals, if any, are combined into the same request.
- **Cert parameters** come from `self_principal.certificate_rules` when the caller's own uid is granted alone, or from the matching group's `certificate_rules` when combined with a group grant; in both cases `self_principal.certificate_rules.allowed_principals` is ignored (the principal is the uid).
...
`--lsm-monitor`/`--lsm-enforce` (docs/vsock-connect-detection.md §4.6) need real kernel support beyond what the
tracepoint detectors require — check before enabling either flag:

```bash
# CONFIG_BPF_LSM must be built in:
grep CONFIG_BPF_LSM /boot/config-$(uname -r)
# "bpf" must be among the ACTIVE LSMs (not just compiled in):
cat /sys/kernel/security/lsm
```

If `bpf` is missing from the active LSM list, it must be **appended to** the kernel's `lsm=` boot parameter —
**never replace it outright**: `lsm=bpf` alone silently drops every other active LSM (SELinux, AppArmor), which
