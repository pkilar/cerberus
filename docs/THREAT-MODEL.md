...
82 threats were identified across 9 trust-boundary domains:
...
| `AUTHZ-13` | Low | 3.2 | E | Self-service issuance (`self_principal`) grants a cert for the caller's own short uid — including `root`, with no hardcoded floor — without group membership, via explicit `--self`, an implicit connect for the caller's own uid, or riding along on a group-granted request for other principals |
...
| `AUTHZ-16` | Low | 3.0 | E | A group's `$self` mapping mints a certificate for the caller's own uid without the caller invoking the self-service path, so group membership plus a mapping — not `self_principal: true` — is what triggers uid issuance |
...
- **Existing controls:** The enclave-side validateSigningRequest (ssh-cert-signer/internal/handlers/sign-public-key.go:177-198) checks: (1) non-empty slice, (2) len ≤ messages.MaxPrincipals (messages/messages.go:15, cap=100), (3) no empty/whitespace principal (line 192-193), (4) no literal '*' (line 195-196). The host-side handleSignRequest (ssh-cert-api/internal/api/server.go) authorizes the user via Casbin before constructing the EnclaveSigningRequest, and sends only `result.GrantedPrincipals` — the requested names Casbin confirmed for the matched group, each resolved through that group's `allowed_principals` mapping (`root: global-root` issues `global-root`) and deduplicated in `authz.Authorize`; the handler never derives principals from the request body. The enclave has no independent view of the Kerberos-to-group mapping; it cannot re-authorize the request. An opt-in, monitor-first preventive control also exists at the VSOCK transport layer: `cerberus-vsock-watch --lsm-enforce` (docs/vsock-connect-detection.md §4.6) attaches a BPF_PROG_TYPE_LSM program to the socket_connect hook that can deny a connect() to the enclave from a process outside ssh-cert-api's own cgroup, before the connect succeeds. **This narrows, but does not close, SIGN-1**: it stops a rogue, non-ssh-cert-api process from reaching the enclave at all, but a compromise of ssh-cert-api itself (or anything sharing its cgroup) still passes the gate untouched and can send an arbitrary EnclaveSigningRequest — the LSM hook has no way to inspect the request payload, only the identity of the connecting process, and only by cgroup (not exe/uid, which would require a synchronous userspace round-trip an LSM hook cannot make).
- **Residual risk:** A fully compromised host process (e.g., supply-chain attack on the binary, or host OS takeover) can send arbitrary principals to the enclave with no further check. The enclave's principal validation only enforces structural properties (non-empty, no wildcard), not policy. This is an accepted architectural residual: the attestation + PCR0 model protects the CA key itself but not the content of signing requests on a compromised host. Residual risk is rated High because the VSOCK channel is the only control plane. The LSM gate above is a narrowing, opt-in mitigation, not a fix — the rating is unchanged.
...
#### `SC-6` — encrypt-ca-key Makefile target materializes the plaintext CA key during generation before KMS encryption
**High** · DREAD 6 · STRIDE: Tampering, InformationDisclosure

- **Attack:** The `make encrypt-ca-key` target (the `encrypt-ca-key` recipe in ssh-cert-signer/Makefile) generates the SSH CA private key with `ssh-keygen` and encrypts it with `aws kms encrypt` before deleting the plaintext. `ssh-keygen` has no "emit private key to stdout" mode, so the plaintext CA key is necessarily materialized as a `0600` file during the window between generation and deletion. If that file lands on durable storage, simple deletion is not a guaranteed erase on SSDs (wear-leveling), copy-on-write, or journaling filesystems, so the key material may persist in unallocated blocks, snapshots, or backups. Independently, if the build host is accessible to other processes (shared CI runner, container with mounted volumes, or an attacker with concurrent read access), the plaintext could be read during its lifetime.
- **Existing controls:** The Makefile now generates and encrypts the CA key inside a **RAM-backed working directory** (`mktemp -d -p /dev/shm`, tmpfs on Linux) whenever `/dev/shm` is available, so on the default Linux path the plaintext private key **never touches durable storage**. Only the two safe artifacts (KMS-encrypted `ca_key.enc` and public `ca_key.pub`) are moved into the working tree; a single-shell `trap ... EXIT INT TERM` shreds and removes the tmpfs plaintext on every exit path, including a KMS failure or Ctrl-C. Where `/dev/shm` is unavailable (e.g. macOS) the target warns and falls back to the working directory with `shred -u ca_key 2>/dev/null || rm -f ca_key` (matching the README/RUNBOOK manual recipes, which were updated to use the same tmpfs pattern). root .gitignore:33 (`ca_key.*`) excludes all ca_key variants from git tracking. The target also **refuses to run when any CA key material already exists**, so it cannot silently clobber a live CA (and `make clean` no longer deletes key material). Key generation is a one-time operator procedure run on a trusted host, not in CI. The EIF bakes the encrypted ca_key.enc, not the plaintext.
- **Residual risk:** On the default Linux path the durable-storage vector is closed — the plaintext lives only in tmpfs (RAM) and is wiped by the cleanup trap, so there are no unallocated blocks, snapshots, or backups to recover from. Two residuals remain: (1) the `/dev/shm`-absent fallback path still writes the plaintext to durable storage and relies on `shred`, which is not a guaranteed erase on log-structured/CoW filesystems or SSDs — prefer a Linux host with tmpfs; (2) RAM-backing does **not** defend against a concurrent same-host reader during the plaintext's lifetime, since the tmpfs file has the same `0600` permissions and path visibility as a disk file — the only local principal that can win that race against a `0600` file is root, which also defeats tmpfs (it can read `/proc/<pid>/mem` or ptrace `ssh-keygen`). Fully closing that window would require generating the CA key **inside the enclave** so the plaintext never exists outside NSM-attested memory; the current control is procedural (run on a trusted, single-tenant host).
- **DREAD:** Damage 9 · Reproducibility 4 · Exploitability 3 · Affected 9 · Discoverability 3 *(Discoverability lowered 5→3: the recoverable durable-storage artifact is eliminated on the default Linux/tmpfs path, leaving only the live same-host read window and the non-tmpfs fallback; Damage stays 9 because a hit still means full CA-key compromise, so the finding remains High)*
...
| `AUTHZ-12` | Medium | 5.4 | E | Literal '*' principal rejected client-side but no server-side guard | Explicit `if p=="*"` 400 check `server.go:232-237`; cert issued for the requested principals (or their configured mapping) only; `*` is never issued and may be neither key nor target of a mapping (`config.Validate`) |
...
| `LOG-2` | Medium | 5.4 | I | group_allowed_principals logged at Info on every successful sign | Intentional audit field (`sign.success`, CLAUDE.md-documented) so operators can audit dynamic group assignment; the same event logs `requested_principals` and `granted_principals` so a principal-mapping rewrite is visible on one line |
...
| `AUTHZ-13` | Low | 3.2 | E | Self-service issuance (`self_principal`) grants a cert for the caller's own short uid without group membership — via explicit `--self`, an implicit connect for the caller's own uid, or riding along inside a mixed request like `["root", "<own uid>"]` that a group otherwise covers. There is no hardcoded `root` floor: a caller whose own uid is `root` (e.g. a `root@REALM` Kerberos principal used for host-to-host automation) self-issues a `root` cert by default — this is the intended mechanism for replacing static root SSH keys, so it is a deliberate widening of what this path grants, not an oversight. `self_principal.deny` is keyed on the requesting caller's own uid, not on principal names appearing elsewhere in a request, so it never blocks a group from granting `root` (or any other principal) to a different caller. | Opt-in, off by default; realm allowlist blocks the cross-realm uid collision; operator-configured denylist (`self_principal.deny`, `selfDenySet` in `casbin.go`) lets an operator still exclude `root` or any other uid if they don't want this path to cover it; cert scoped to exactly the requested principals; the implicit connect path only self-accepts alone when the requested principal set is **exactly** the caller's own uid (`server.go` fallback); when combined with other principals, `Authorize` (`casbin.go`) still requires every principal OTHER than the caller's own uid to be covered by a single group (`selfEligibleUID` gate, `sawGroupCheckedPrincipal` guard prevents a solo self request from trivially matching an unrelated group) — a principal that is neither the caller's own uid nor covered by that one group is still refused; sshd is still the final gate (the account must be named/mapped to the principal — pair with `AuthorizedPrincipalsFile`). `config.Validate()` requires realms+validity when enabled; `AuthorizeSelf` + `Authorize` + fallback unit-tested (realm allowlist, denylist, root allowed by default, root deniable via explicit denylist entry, requested==uid, self-augmented group grant, solo-self-uid not satisfied by an unrelated group) |
...
| `AUTHZ-16` | Low | 3.0 | E | A group's `$self` mapping issues the caller's own uid without an explicit self-service request | Resolution runs through the same `selfEligibleUID` gate as the self path (`self_principal.enabled`, `realms` allowlist, `deny` list), so no uid is issuable that `self_principal: true` could not already obtain; a group using `$self` with the block disabled is refused at config load (`config.Validate`); `$self` is never a Casbin policy object and `/sign` refuses a requested principal beginning with `$`, so it cannot be asked for; an ineligible caller is a coverage miss logged as `authz.self_target.ineligible`, never a silent grant; `TestAuthorize_SelfTarget*`, `TestValidate_SelfTargetRequiresSelfPrincipal`, `TestHandleSignRequest_ReservedPrincipalRejected` |
