...
type Authorizer interface {
	...
	// for the matched group. The caller's own self-issuable uid (per
	// self_principal, when enabled) is treated as automatically satisfied by
	// any group that covers the rest of the request — self_principal is
	// independent of group membership, so it must never be the reason a
	// mixed request like ["root", "<own uid>"] is denied. ctx is propagated
	// to any directory-service lookups; implementations that do not perform
	// I/O may ignore it.
