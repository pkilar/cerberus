...
// CA_PUBLIC_KEY_PATH is set, verifies the resulting public key against the pinned
// CA public key baked into the (PCR0-measured) enclave image. The pin is opt-in
// defense-in-depth against ciphertext substitution — a compromised host feeding
// the enclave a CiphertextForRecipient derived from a substituted ciphertext,
// which is only feasible if the CMK grants kms:Encrypt. The primary control is a
// least-privilege, Decrypt-only KMS key policy; see docs/kms-attestation-policy.md.
...
// by PCR0), so a host cannot tamper with it. When unset it warns and proceeds —
// the pin is opt-in defense-in-depth on top of the KMS key policy.
